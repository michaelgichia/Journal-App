      ...
      return (
        <button
          ...
          ${
            dayData.isSelected
              ? 'text-white'
              : dayData.isToday
              ? 'text-teal-500' // Changed for visibility
              : 'text-gray-700'
          }
        ...
        >
          ...
          {dayData.isSelected && !dayData.isDisabled && (
            <div className='absolute inset-0 bg-teal-500 rounded-full z-[-1]' /> // Changed for visibility
          )}
          {dayData.isToday && !dayData.isSelected && !dayData.isDisabled && (
            <div className='absolute bottom-0 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-teal-500 rounded-full' /> // Changed for visibility
          )}
        </button>
      )
    },
    ...
    <div className='relative flex flex-col gap-2 w-full' ref={datePickerRef}>
      ...
      <div className='relative' onKeyDown={handleKeyDown} role='presentation'>
        <input
          ...
          className={`w-full p-2 pr-8 rounded-md bg-white border focus:ring-offset-0 border-gray-300 focus-visible:border-teal-500 focus:border-teal-500 focus:ring-teal-500/20 focus:outline-none focus:ring cursor-pointer`} //changed
        />
        <span
